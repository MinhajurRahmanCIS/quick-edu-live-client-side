...
const Assignments = () => {
    ...
    const { data: assignments = [], isLoading: assignmentsIsLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`https://quick-edu-live-server-side.vercel.app/classwork?classId=${id}&assignmentNo=true`, {
                headers: {
    ...
    const { data: assignmentSubmissions = [], isLoading: assignmentSubmissionsLoading } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`https://quick-edu-live-server-side.vercel.app/checkSubmission?email=${user?.email}&assignment=true`, {
                headers: {
                    authorization: `bearer ${localStorage.getItem("quickEdu-token")}`
                }
            });
